 ...
 * PhoneNumberMetaData.  Certain elements names are changed to work with
 * the PhoneNumber validator.
 */
...
foreach ($assoc as $territory) {
    ...
    foreach ($patterns as $p) {
        ...
        foreach ($p['childs'] as $pattern) {
            ...
            if ($pattern['tag'] == 'nationalNumberPattern') {
                $pattern['tag'] = 'national';
            }
            if ($pattern['tag'] == 'possibleNumberPattern') {
                $pattern['tag'] = 'possible';
            }
...
foreach ($phone as $k => $v) {
    ...
    echo "\t\t\t'national' => array(" . PHP_EOL;
    foreach ($v['patterns']['national'] as $kk => $vv) {
        echo "\t\t\t\t'$kk' => '$vv'," . PHP_EOL;
    }
    echo "\t\t\t)," . PHP_EOL;
    echo "\t\t\t'possible' => array(" . PHP_EOL;
    foreach ($v['patterns']['possible'] as $kk => $vv) {
        echo "\t\t\t\t'$kk' => '$vv'," . PHP_EOL;
    }
    echo "\t\t\t)," . PHP_EOL;
    echo "\t\t)," . PHP_EOL;
